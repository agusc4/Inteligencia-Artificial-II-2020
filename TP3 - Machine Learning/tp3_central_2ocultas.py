...
"QUIZÁS"
    ...
    "- Implementar una segunda capa oculta. Ambas capas pueden usar ReLU o Sigmoide como
    función de activación."
    "- Realizar REGULARIZACIÓN (L2 o Dropout) en lugar de Cross-Validation para evitar el Overfitting"
 """
...
def inic_pesos(n_entrada, n_capa_oc):
    ...
    w2 = 0.1 * randomgen.standard_normal((n_capa_oc,n_capa_oc))
    b2 = 0.1 * randomgen.standard_normal((1,n_capa_oc))

    w3 = 0.1 * randomgen.standard_normal((n_capa_oc,1))
    b3 = 0.1 * randomgen.standard_normal((1,1))
...
def ejec_forw(x,pesos):
    ...
    # Funcion de activacion capa oculta 1 ( h=f(z) )
    h = 1/(1+np.exp(-z)) #Sigmoide
    #h = np.maximum(0, z) #ReLU
    # Funcion de entrada (a.k.a. "regla de propagacion") para la segunda capa oculta
    g = h.dot(pesos["w2"]) + pesos["b2"]
    # Funcion de activacion capa oculta 2 ( u=f(g) )
    u = 1/(1+np.exp(-g)) #Sigmoide
    #u = np.maximum(0, g) #ReLU
    # Salida de la red (funcion de activacion lineal).
    y = h.dot(pesos["w3"]) + pesos["b3"]
    
    return {"z": z, "h": h, "g": g, "u": u, "y": y}
...
def train(x, t,xv,tv, pesos, l_rate, epochs,nv):
    ...
    for i in range(epochs+1):
        ...
        dL_dy = -2*lw/n
        dL_dw3 = u.T.dot(dL_dy)                         #Ajuste para w3
        dL_db3 = np.sum(dL_dy, axis=0, keepdims=True)   #Ajuste para b3
        dL_du = dL_dy.dot(w3.T)

        # dL_dg = dL_du          ###Para usar ReLU
        # dL_dg[g<=0] = 0        ###  ReLU
        du_dg = u.T.dot((1-u))  ### SIGMOIDE. El calculo dL/dg = dL/du * sigma(x)*(1-sigma(x))
        dL_dg = dL_du.dot(du_dg)### Sigmoide

        dL_dw2 = h.T.dot(dL_dg)                         # Ajuste para w2
        dL_db2 = np.sum(dL_dg, axis=0, keepdims=True)   # Ajuste para b2
        dL_dh = dL_dg.dot(w2.T)        
        ...
        if i %nv == 0:
            ...
            if LWv[k] <= val:          ### Actualiza el valor de "Loss" actual
                ...
                if LWv[k] > (val*2):   ### Corto por Overfitting
                    print("Parada Temprana en epoch", i," por Overfitting (oscilación mayor al 100%).\n")
                    break
                else:
                    ### Advertencia de Overfitting
                    print("Posible Overfitting (oscilación mayor al 50%).")
            if i>0:                     ### No reviso CORRELATION en la primer EPOCH
                if  (er_abs> 0.2):      ### Diferencia del 20% entre Train. y Valid.
                    if er_abs > 1:      ### Diferencia del 100% - Corta por Correlation
                        print("Parada Temprana en epoch", i," por no Correlación\n")
                        break
                    else:
                        ### Advertencia de Correlación
                        print("Posible error de Correlación (mayor al 20%).")
            print("\n")
    plt.figure()
